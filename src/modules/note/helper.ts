...
export const getNotesByMetadataValue = async (space: string, metadataId: string, payload: { value: string }) => {
  const model = getCollection(space, noteCollection, noteSchema);
  let res = [];
  if  (metadataId === 'label') {
    res = await model.find({ labels: new RegExp(payload.value, 'i') }).sort({ createdAt: -1 });
  } else {
    res = await model.find({ [metadataId]: new RegExp(payload.value, 'i') }).sort({ createdAt: -1 });
  }
  return res.map((item: any) => {
    return {
      ...item._doc,
